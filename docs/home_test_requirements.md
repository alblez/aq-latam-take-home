...
Deterministic interviewer state: Show a "decision panel" that displays the interviewer's current rubric/signals (skills detected, topics covered, gaps) and why it chose the next question.

Job-specific question packs: Add structured question banks per role (behavioral + technical categories), and have the AI select from them + generate targeted follow-ups.

Video mode: Add optional camera input and have the interview run in a "video call" layout (still voice-driven; video is just the interface/UX layer).

Replay + analytics: Add a session history page where users can replay interviews, filter by role, and see per-session metrics (duration, talk ratio, topic coverage, score trend).
