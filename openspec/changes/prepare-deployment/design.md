...
- Production configuration that points the frontend at the deployed backend and restricts CORS to the frontend origin.
...
- **Production points at the deployed backend.** The frontend is deployed with `NEXT_PUBLIC_API_URL` / `API_URL` pointing at the deployed backend; the backend restricts `CORS_ALLOWED_ORIGINS` to the frontend origin.
...
3. Deploy the frontend pointing at the deployed backend.
